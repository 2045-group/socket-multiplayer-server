...
app.use(
  cors({
    origin: ["http://localhost:5173"], // строго совпадает
    credentials: true,
  })
);
const io = new Server(server, {
  cors: {
    origin: [
      "http://localhost:5173",
      "https://one087-socket-multiplayer.onrender.com/",
    ],
    methods: ["GET", "POST"],
    credentials: false,
  },
...
io.on("connection", (socket) => {
  ...
  socket.on("move_user", (movement) => {
    ...
    if (userIndex !== -1) {
      ...
      newX = Math.max(
        bubbleSize / 2,
        Math.min(GAME_WIDTH - bubbleSize / 2, newX)
      );
      newY = Math.max(
        bubbleSize / 2,
        Math.min(GAME_HEIGHT - bubbleSize / 2, newY)
      );
      ...
      for (let i = 0; i < users.length; i++) {
        if (i !== userIndex) {
          ...
          if (collision.canEat) {
            const eaterIndex = users.findIndex(
              (u) => u.id === collision.eater.id
            );
            const victimIndex = users.findIndex(
              (u) => u.id === collision.victim.id
            );
            ...
            console.log(
              `${collision.eater.name} (${
                users[eaterIndex].score - victimScore
              } -> ${users[eaterIndex].score}) ate ${
                collision.victim.name
              } (${victimScore})`
            );
